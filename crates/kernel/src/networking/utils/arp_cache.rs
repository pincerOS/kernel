...
// use crate::networking::utils::arp_cache::Timestamp;
...
impl ArpCache {
    ...
    pub fn set_eth_addr_for_ip(&mut self, ipv4_addr: Ipv4Address, eth_addr: EthernetAddress) {
        ...
        if self.entries.is_empty() {
            self.in_cache_since_min = Timestamp(now);
        }
    ...
    fn expire_eth_addr(&mut self) {
        ...
        if now > self.in_cache_since_min.0 + (self.expiration.as_micros() as u64) {
            let expiration = self.expiration;
            self.entries.retain(|_, entry| now.saturating_sub(entry.in_cache_since.0) <= (expiration.as_micros() as u64));
