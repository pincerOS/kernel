...
impl DhcpClient {
    ...
    pub fn release(&mut self, interface: &mut Interface) -> Result<()> {
        if self.state != DhcpState::Bound && self.state != DhcpState::Renewing && self.state != DhcpState::Rebinding {
            return Ok(());
        }
    ...
    pub fn process_dhcp_packet(&mut self, interface: &mut Interface, packet: DhcpPacket, current_time: u64) -> Result<()> {
        ...
        match (self.state, msg_type) {
            ...
            (DhcpState::Rebinding, DhcpMessageType::Ack) => {
                ...
                info!("DHCP: Bound to IP {} with lease time {} seconds", packet.yiaddr, 
            ...
            _ => {
                ...
                debug!("DHCP: Ignoring message type {:?} in state {:?}", msg_type, self.state);
                return Err(Error::Ignored);
            }
        }
