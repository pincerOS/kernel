...
fn HubReadDescriptor(device: &mut UsbDevice) -> ResultCode {
    ...
    let mut hub = unsafe { &mut *(device.driver_data.unwrap() as *mut HubDevice) };
    if hub.Descriptor == core::ptr::null_mut() {
        hub.Descriptor = vec!(header.descriptor_length as usize).as_mut_ptr() as *mut HubDescriptor;
    }

    result = UsbGetDescriptor(device, DescriptorType::Hub, 0, 0, hub.Descriptor as *mut u8, header.descriptor_length as u32, header.descriptor_length as u32, 0x20);
...
fn HubPowerOn(device: &mut UsbDevice) -> ResultCode {
    let data = unsafe { &mut *(device.driver_data.unwrap() as *mut HubDevice) };
    let hub_desc =  unsafe { &mut *(data.Descriptor) };
...
fn HubPortReset(device: &mut UsbDevice, port: u8) -> ResultCode {
    let mut data = unsafe { &mut *(device.driver_data.unwrap() as *mut HubDevice) };
...
fn HubAttach(device: &mut UsbDevice, interface_number: u32) -> ResultCode {
    ...
    device.device_child_reset = None; //TODO: Implement
    device.device_check_connection = Some(HubCheckConnectionDevice);

    let mut driver_data = vec!(size_of::<HubDevice>());
    device.driver_data = Some(driver_data.as_mut_ptr() as *mut u8);

    let mut hub = unsafe { &mut *(device.driver_data.unwrap() as *mut HubDevice) };

    hub.Header.data_size = size_of::<HubDevice>() as u32;
    hub.Header.device_driver = DeviceDriverHub;
    hub.Descriptor = core::ptr::null_mut();
    ...
    let mut HubDescriptor = unsafe { &mut *(hub.Descriptor) };
    ...
    println!("| HUB: hub has {} ports", hub.MaxChildren);
