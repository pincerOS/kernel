...
impl bcm2711_emmc2_driver {
    ...
    fn issue_command_int(&mut self, cmd_reg: u32, argument: u32, timeout: u32) {
        ...
        if irpts & 0xffff0001 != 1 {
            ...
            println!("returning 977, irpts: {:#010x}", irpts);
            return;
        }
        unsafe {
    ...
    fn card_reset(&mut self) -> Result<(), SdCardError> {
        unsafe  {
            ...
            if self.timeout_wait(STATUS, 1 << 16, 1, 1000000).is_err() {
    ...
    fn do_write(&mut self, buf: &mut [u8], block_no: u32) -> Result<u32, SdCardError> {
        if self.ensure_data_mode().is_err() {
            return Err(SdCardError::Error);
        }

        if self.do_data_command(true, buf, block_no).is_err() {
            return Err(SdCardError::Error);
        }
