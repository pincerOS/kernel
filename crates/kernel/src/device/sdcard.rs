...
// TODO:
// Get it working !! :(
// See if it works on hardware, this implementation seems more specific to
// the raspberry pi 4's emmc2 controller but a more general driver could possibly work.
// qemu recognizes the emmc2 address space however it needs sdhci compatibility.
...
use crate::sync::{spin_sleep, Volatile};
...
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub enum SdErrMask {
    CmdTimeout = 1 << (16 + SD_ERR_CMD_TIMEOUT),
    CmdCrc = 1 << (16 + SD_ERR_CMD_CRC),
    CmdEndBit = 1 << (16 + SD_ERR_CMD_END_BIT),
    CmdIndex = 1 << (16 + SD_ERR_CMD_INDEX),
    DataTimeout = 1 << (16 + SD_ERR_DATA_TIMEOUT),
    DataCrc = 1 << (16 + SD_ERR_DATA_CRC),
    DataEndBit = 1 << (16 + SD_ERR_DATA_END_BIT),
    CurrentLimit = 1 << (16 + SD_ERR_CURRENT_LIMIT),
    AutoCmd12 = 1 << (16 + SD_ERR_AUTO_CMD12),
    Adma = 1 << (16 + SD_ERR_ADMA),
    Tuning = 1 << (16 + SD_ERR_TUNING),
}
...
// bitflags! {
//     pub struct SdResponseType: u32 {
//         const NONE = SD_CMD_RSPNS_TYPE_NONE;
//         const R1   = SD_CMD_RSPNS_TYPE_48 | SD_CMD_CRCCHK_EN;
//         const R1b  = SD_CMD_RSPNS_TYPE_48B | SD_CMD_CRCCHK_EN;
//         const R2   = SD_CMD_RSPNS_TYPE_136 | SD_CMD_CRCCHK_EN;
//         const R3   = SD_CMD_RSPNS_TYPE_48;
//         const R4   = SD_CMD_RSPNS_TYPE_136;
//         const R5   = SD_CMD_RSPNS_TYPE_48 | SD_CMD_CRCCHK_EN;
//         const R5b  = SD_CMD_RSPNS_TYPE_48B | SD_CMD_CRCCHK_EN;
//         const R6   = SD_CMD_RSPNS_TYPE_48 | SD_CMD_CRCCHK_EN;
//         const R7   = SD_CMD_RSPNS_TYPE_48 | SD_CMD_CRCCHK_EN;
//     }
// }
...
// impl SdResponseType {
//     pub const NONE: Self = Self(SD_CMD_RSPNS_TYPE_NONE);
//     pub const R1: Self = Self(SD_CMD_RSPNS_TYPE_48 | SD_CMD_CRCCHK_EN);
//     pub const R1b: Self = Self(SD_CMD_RSPNS_TYPE_48B | SD_CMD_CRCCHK_EN);
//     pub const R2: Self = Self(SD_CMD_RSPNS_TYPE_136 | SD_CMD_CRCCHK_EN);
//     pub const R3: Self = Self(SD_CMD_RSPNS_TYPE_48);
//     pub const R4: Self = Self(SD_CMD_RSPNS_TYPE_136);
//     pub const R5: Self = Self(SD_CMD_RSPNS_TYPE_48 | SD_CMD_CRCCHK_EN);
//     pub const R5b: Self = Self(SD_CMD_RSPNS_TYPE_48B | SD_CMD_CRCCHK_EN);
//     pub const R6: Self = Self(SD_CMD_RSPNS_TYPE_48 | SD_CMD_CRCCHK_EN);
//     pub const R7: Self = Self(SD_CMD_RSPNS_TYPE_48 | SD_CMD_CRCCHK_EN);
...
pub const fn sd_cmd_reserved(a: u32) -> u32 {
    0xffffffff
}
...
static mut emmc_base: u32 = EMMC_BASE;

pub fn emmc_set_base(base: u32) {
    unsafe {
        emmc_base = base;
    }
}

static SD_VERSIONS: [&str; 6] = [
    "unknown",
    "1.0 and 1.01",
    "1.10",
    "2.00",
...
#[cfg(feature = "emmc_debug")]
static ERR_INTERRUPTS: [&str; 12] = [
    "CMD_TIMEOUT",
    "CMD_CRC",
    "CMD_END_BIT",
    "CMD_INDEX",
...
// impl SdCommand {
//     /// Returns an alternative name for duplicate values.
//     pub fn alias(self) -> &'static str {
//         match self {
//             SdCommand::SelectDeselectCard => "SelectCard / DeselectCard / SelectDeselectCard",
//             _ => "Unique Command",
//         }
//     }
// }
...
// const CMD_NEED_APP: usize = 0x80000000;
// const CMD_RSPNS_48: usize = 0x00020000;
// const CMD_ERRORS_MASK: usize = 0xfff9c004;
// const CMD_RCA_MASK: usize = 0xffff0000;
...
impl bcm2711_emmc2_driver {
    ...
    fn initialize(&mut self) -> Result<(), SdCardError> {
        // if self.power_off().is_err() {
        //     sderror!("Initial power disable failed")
        // }
    ...
    fn power(&mut self, val: u32) -> Result<(), SdCardError>{
        const BUFFER_WORDS: usize = 8;
        let mut buffer = [0u128; BUFFER_WORDS / 4];
        let words: &mut [u32; BUFFER_WORDS] = bytemuck::cast_slice_mut::<_, u32>(&mut buffer)
            .try_into()
        ...
        println!("Power value {}", words[6] & 0x3);
        Ok(())
    }
    ...
    fn get_clock_divider(&mut self, base_clock: u32, target_rate: u32) -> Result<u32, SdCardError>{
        ...
        for first_bit in (0..=31).rev() {
            let bit_test: u32 = 1 << first_bit;
            if targetted_divisor & bit_test != 0 {
                divisor = first_bit as i32;
    ...
    fn switch_clock_rate(&mut self, base_clock: u32, target_rate: u32) -> Result<(), SdCardError> {
        ...
        let divider = self.get_clock_divider(base_clock, target_rate).unwrap();
    ...
    fn timeout_wait(&mut self, reg: usize, mask: u32, value: u32, usec: u32) -> Result<(), SdCardError>{
    ...
    //TODO: dma?
    fn issue_command_int(&mut self, cmd_reg: u32, argument: u32, timeout: u32) {
        ...
        if self.emmc.blocks_to_transfer > 0xffff {
            self.emmc.last_cmd_success = 0;
            println!("blocks_to_transfer too great {}", self.emmc.blocks_to_transfer);
            return;
        }
        // unsafe {println!("interrupt register pre sending command: {:#010x}", self.reg(INTERRUPT).read());}
        let blksizecnt: u32 = self.emmc.block_size as u32 | (self.emmc.blocks_to_transfer << 16) as u32;
        ...
        spin_sleep(1000000);
        self.timeout_wait(INTERRUPT, 0x8001, 1, timeout);
        ...
        if cmd_reg & SD_CMD_ISDATA != 0 {
            ...
            for nBlock in 0..self.emmc.blocks_to_transfer {
                self.timeout_wait(INTERRUPT, wr_irpt | 0x8000, 1,  timeout as u32);
                unsafe {
        ...
        if (cmd_reg & SD_CMD_RSPNS_TYPE_MASK == SD_CMD_RSPNS_TYPE_48B) || (cmd_reg & SD_CMD_ISDATA != 0) {
            unsafe {
                self.timeout_wait(INTERRUPT, 0x8002, 1, timeout);
                irpts = self.reg(INTERRUPT).read();
    ...
    fn handle_card_interrupt(&mut self) {
        if self.emmc.card_rca != CARD_RCA_INVALID {
            self.issue_command_int(sd_commands[SEND_STATUS as usize], self.emmc.card_rca << 16, 500000);
            if self.emmc.fail() {
    ...
    fn issue_command(&mut self, mut command: u32, argument: u32, timeout: u32) -> bool {
        ...
        if command & IS_APP_CMD != 0 {
            ...
            self.issue_command_int(sd_commands[APP_CMD as usize], rca, timeout);
            if self.emmc.last_cmd_success != 0 {
                self.emmc.last_cmd = command | IS_APP_CMD;
                self.issue_command_int(sd_acommands[command as usize], argument, timeout);
            } else {
            ...
            self.issue_command_int(sd_commands[command as usize], argument, timeout);
        }

        return self.emmc.last_cmd_success != 0;
    }
    ...
    fn card_reset(&mut self) -> Result<(), SdCardError> {
        unsafe  {
            ...
            let f_id = self.get_clock_divider(base_clock, SD_CLOCK_ID).unwrap();
            ...
            if !(self.issue_command(GO_IDLE_STATE, 0, 500000)) {
                println!("No CMD0 response");
                return Err(SdCardError::Error);
            }

            if !self.issue_command(SEND_IF_COND, 0x1aa, 500000) {
                println!("CMD8 error");
            }
            let mut v2_later = 0;
            ...
            self.issue_command(IO_SET_OP_COND, 0, 100000);
            if !(self.emmc.timeout()) {
            ...
            if !(self.issue_command(acmd(41), 0, 100000000)) {
            ...
            while card_is_busy != 0 {
                ...
                if !(self.issue_command(acmd(41), 0x00ff8000 | v2_flags, 500000)) {
            ...
            self.switch_clock_rate(base_clock, SD_CLOCK_NORMAL);

            spin_sleep(5000);

            if self.emmc.card_supports_18v != 0 {
                if !(self.issue_command(VOLTAGE_SWITCH, 0, 500000)) {
            ...
            if !(self.issue_command(ALL_SEND_CID, 0, 100000000)) {
            ...
            if !(self.issue_command(SEND_RELATIVE_ADDR, 0, 500000)) {
            ...
            if !(self.issue_command(SEND_CSD, self.emmc.card_rca << 16, 500000)) {
                sderror!("Error sending CMD9")
            }
            ...
            println!("SD capacity is {:?} MBytes", self.emmc.capacity / 100000);

            if !(self.issue_command(SELECT_CARD, self.emmc.card_rca << 16, 500000)) {
                sderror!("Error sending CMD7")
            }

            let cmd7_reso = self.emmc.last_r0;
            status = (cmd7_reso >> 9) & 0xf;
            if status != 3 && status != 4 {
                sderror!("Invalid status")
            }

            if self.emmc.card_supports_sdhc == 0 {
                if !(self.issue_command(SET_BLOCKLEN, SD_BLOCK_SIZE, 500000)) {
                    sderror!("Error sending SET_BLOCKLEN")
                }
            }
            ...
            self.issue_command(SEND_SCR, 0, 1000000);
            self.emmc.block_size = SD_BLOCK_SIZE;
            if self.emmc.fail() {
                return Err(SdCardError::Error2);
            }

            let scr0: u32 = (*self.emmc.scr).scr[0].swap_bytes();
            (*self.emmc.scr).sd_version = SD_VER_UNKNOWN;
            ...
            if sd_spec == 0 {
                (*self.emmc.scr).sd_version = SD_VER_1;
            } else if sd_spec == 1 {
                (*self.emmc.scr).sd_version = SD_VER_1_1;
            } else if sd_spec == 2 {
                if sd_spec3 == 0 {
                    (*self.emmc.scr).sd_version = SD_VER_2;
                } else if sd_spec3 == 1 {
                    if sd_spec4 == 0 {
                        (*self.emmc.scr).sd_version = SD_VER_3;
                    } else if sd_spec4 == 1 {
                        (*self.emmc.scr).sd_version = SD_VER_4;
                    }
                }
            }
            ...
            if  (*self.emmc.scr).sd_version >= SD_VER_1_1 {
                ...
                if !(self.issue_command(SWITCH_FUNC, 0x00fffff0, 100000)) {
                    ...
                    if self.emmc.card_supports_hs != 0 {
                        if !(self.issue_command(SWITCH_FUNC, 0x80fffff1, 100000)) {
                            println!("Switch failed");
                        } else {
                            self.switch_clock_rate(base_clock, SD_CLOCK_HIGH);
                        }
                    }
                }
                self.emmc.block_size = SD_BLOCK_SIZE;
            ...
            if (*self.emmc.scr).sd_bus_widths & 4 != 0 {
                ...
                if !(self.issue_command(SET_BUS_WIDTH, 2, 500000)) {
            ...
            println!("Found a valid {:#?} SD Card", SD_VERSIONS[(*self.emmc.scr).sd_version as usize]);
    ...
    fn card_init(&mut self) -> Result<(), SdCardError> {
        // if self.power_off().is_err() {
        //     sderror!("SD Card Controller did not power off successfully")
        // }
        // spin_sleep(5000);
        ...
        // // TODO: error handling
        // let response = words[1];
        // println!("words 4 {:#010x}", words[4]);
        // println!("Mailbox response: {:#010x}", response);
        // println!("Device id: {}", words[5]);
        // println!("State: {}", words[6]);
        ...
        println!("SD version: {:#?}, vendor: {:#010x}, slot status: {:#010x}", SD_VERSIONS[sdversion as usize], vendor, slot_status);
        ...
        for _ in 0..3 {
            ret = self.card_reset();
            match ret {
                Err(SdCardError::Error2) => println!("Card reset failed. Retrying."),
                _ => break,
            }
        }
    ...
    fn ensure_data_mode(&mut self) -> Result<(), SdCardError> {
        ...
        if !(self.issue_command(SEND_STATUS, self.emmc.card_rca << 16, 500000)) {
            self.emmc.card_rca = CARD_RCA_INVALID;
            sderror!("ensure_data_mode() error sending CMD13")
        }
        ...
        if cur_state == 3 {
            if !(self.issue_command(SELECT_CARD, self.emmc.card_rca << 16, 500000)) {
                self.emmc.card_rca = CARD_RCA_INVALID;
                sderror!("ensure_data_mode() no response from CMD17")
            }
        } else if cur_state == 5 {
            if !(self.issue_command(STOP_TRANSMISSION, 0,500000)) {
                self.emmc.card_rca = CARD_RCA_INVALID;
                sderror!("ensure_data_mode() no response from CMD12")
            }
            self.reset_dat();
        } else if cur_state != 4 {
            let ret = self.card_reset();
    ...
    fn do_data_command(&mut self, is_write: bool, buf: &mut [u8], mut block_no: u32) -> Result<(), SdCardError>{
        if self.emmc.card_supports_sdhc == 0 {
        ...
        if buf.len() % self.emmc.block_size as usize != 0 {
            sderror!("do_data_command() called with buffer size not an exact multiple of block size")
        }
        ...
        while retry_count < max_retries {
            if self.issue_command(command, block_no, 500000) {
                break;
            } else {
    ...
    fn do_read(&mut self, buf: &mut [u8], block_no: u32) -> Result<u32, SdCardError> {
        ...
        if self.do_data_command(false, buf, block_no).is_err() {
            return Err(SdCardError::Error);
        }
    ...
    fn do_write(&mut self, buf: &[u8], block_no: u32) -> Result<u32, SdCardError> {
        ...
        if self.do_data_command(true, tmp, block_no).is_err() {
            return Err(SdCardError::Error);
        }
    ...
    pub fn read(&mut self, buf: &mut [u8]) -> Result<u32, SdCardError> {
        ...
        if self.do_read(buf, nBlock).is_ok_and(|x| x != buf.len() as u32) {
    ...
    pub fn write(&mut self, buf: &[u8]) -> Result<u32, SdCardError> {
        ...
        if self.do_write(buf, nBlock).is_ok_and(|x| x != buf.len() as u32) {
            sderror!("write() returned value different than requested bytes")
        }

        Ok(buf.len() as u32)
    }

    pub fn seek(&mut self, offset: u64) -> Result<u64, SdCardError> {
        // if offset % SD_BLOCK_SIZE as u64 != 0 {
        //     sderror!("seek() called with offset not a multiple of block size")
        // }

        self.emmc.ullOffset = offset;
        Ok(self.emmc.ullOffset)
    }
...
impl BlockDevice for bcm2711_emmc2_driver {
    ...
    fn read_sector(
        ...
        self.read(buffer).map_err(|_| filesystem::BlockDeviceError::Unknown)?;
        Ok(())
    }
    ...
    fn write_sector(
        ...
        self.seek(index * filesystem::SECTOR_SIZE as u64);
        self.write(buffer).map_err(|_| filesystem::BlockDeviceError::Unknown)?;
        Ok(())
    }
}
