...
impl Process {
    ...
    pub fn unmap_memory_range(&mut self, addr: usize) -> Result<(), MappingError> {
        let range_node: &MemoryRangeNode = self.get_memory_range_node(addr)?;
        let range_start: usize = range_node.start;
        let range_end: usize = range_start + range_node.size;

        for i in (range_start..range_end).step_by(USER_PG_SZ) {
            unsafe { unmap_va_user(i, self.ttbr0_el1)? };
        }
