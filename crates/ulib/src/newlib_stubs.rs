...
const HEAP_BASE: usize = 5 << 28;

#[unsafe(no_mangle)]
unsafe extern "C" fn _sbrk(incr: usize) -> *mut () {
    if HEAP_END.load(Ordering::SeqCst) == 0 {
        // let heap_base = (&raw const _end).addr().next_multiple_of(65536);
        let _ = HEAP_END.compare_exchange(0, HEAP_BASE, Ordering::SeqCst, Ordering::SeqCst);
    }

    // TODO: not reentrant
    let cur_end = HEAP_END.load(Ordering::SeqCst);

    if incr == 0 {
        return cur_end as *mut ();
    }

    let amount = incr.next_multiple_of(4096);
    let res = super::sys::mmap(cur_end, amount, 0, 0b11, 0, 0);
    match res {
        Err(e) => {
            crate::println!("sbrk({incr}) failed");
            crate::println!("mmap({cur_end:#x}, {amount:#x}, 0, 0b11, 0, 0) -> Err({e})");
            errno = ENOMEM;
            core::ptr::without_provenance_mut(-1isize as usize)
        }
        Ok(ptr) => {
            assert!(ptr.addr() == cur_end);
            HEAP_END.store(cur_end + amount, Ordering::SeqCst);
            crate::println!("sbrk({amount:#x}) -> {cur_end:#010x}");
            let slice = unsafe {
                core::slice::from_raw_parts_mut::<usize>(
                    cur_end as *mut usize,
                    amount / size_of::<usize>(),
                )
            };
            for i in slice {
                // assert!(*i == 0);
