...
fn example_1() {
    ...
    let test_node: Rc<INodeWrapper> = ext2.find(&root_node, b"test.txt").unwrap();
    ...
    let test_folder: Rc<INodeWrapper> = ext2.find(&root_node, b"folder").unwrap();
    let test_file_in_folder = ext2.find(&test_folder, b"asdf.txt").unwrap();
