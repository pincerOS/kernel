...
pub extern "C" fn main() {
    ...
    let mut test_text_file = ulib::sys::openat(root_fd, test_file_path, 0, 0).unwrap();
    ...
    let mut mmap_addr: *mut u8 =
        unsafe { ulib::sys::mmap(0, 4096, 0, 0, test_text_file, 0).unwrap() } as *mut u8;
    ...
    _ = ulib::sys::close(test_text_file);

    println!("starting mmap with offset test 1");
    let test_file_path2 = b"test2.txt";
    test_text_file = ulib::sys::openat(root_fd, test_file_path2, 0, 0).unwrap();
    mmap_addr =
        unsafe { ulib::sys::mmap(0, 4096 * 2, 0, 0, test_text_file, 6).unwrap() } as *mut u8;
    ...
    let shared_frame =
        unsafe { ulib::sys::mmap(0, 4096, 0, 1 << 2, shared_mem_fd, 0) }.unwrap() as *mut u8;
    ...
    _ = unsafe { ulib::sys::munmap(shared_frame as *mut ()) };
