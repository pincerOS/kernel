...
struct UsbDevice {
	...
	void (*DeviceDetached)(struct UsbDevice *device) __attribute__((aligned(8)));
	/** Handler for deallocation of the device. All memory in use by the device driver should be deallocated. */
	void (*DeviceDeallocate)(struct UsbDevice *device) __attribute__((aligned(8)));
	/** Handler for checking for changes to the USB device tree. Only hubs need handle with this. */
	void (*DeviceCheckForChange)(struct UsbDevice *device) __attribute__((aligned(8)));
	/** Handler for removing a child device from this device. Only hubs need handle with this. */
	void (*DeviceChildDetached)(struct UsbDevice *device, struct UsbDevice *child) __attribute__((aligned(8)));
	/** Handler for reseting a child device of this device. Only hubs need handle with this. */
	Result (*DeviceChildReset)(struct UsbDevice *device, struct UsbDevice *child) __attribute__((aligned(8)));
	/** Handler for reseting a child device of this device. Only hubs need handle with this. */
	Result (*DeviceCheckConnection)(struct UsbDevice *device, struct UsbDevice *child) __attribute__((aligned(8)));
	
	volatile struct UsbDeviceDescriptor Descriptor __attribute__((aligned(8)));
	volatile struct UsbConfigurationDescriptor Configuration __attribute__((aligned(8)));
	volatile struct UsbInterfaceDescriptor Interfaces[MaxInterfacesPerDevice] __attribute__((aligned(8)));
	volatile struct UsbEndpointDescriptor Endpoints[MaxInterfacesPerDevice][MaxEndpointsPerDevice] __attribute__((aligned(8)));
	struct UsbDevice *Parent __attribute__((aligned(8)));
	volatile void *FullConfiguration __attribute__((aligned(8)));
	volatile struct UsbDriverDataHeader *DriverData __attribute__((aligned(8)));
	volatile u32 LastTransfer;
};

#include <stddef.h>

_Static_assert(offsetof(struct UsbDevice, Parent) % 8 == 0, "parent misaligned");
