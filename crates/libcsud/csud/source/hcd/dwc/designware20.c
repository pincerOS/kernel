...
void WriteThroughRegMask(volatile const void* reg, u32 maskOr) {
	if ((usize)reg - (usize)Core < sizeof(struct CoreGlobalRegs)) {
		maskOr |= 0xffffffff;
		*(u32*)((usize)reg - (usize)Core + (usize)CorePhysical) = *((u32*)reg) & maskOr;
	} else if ((usize)reg - (usize)Host < sizeof(struct HostGlobalRegs)) {
		switch ((usize)reg - (usize)Host) {
		case 0x40: // Host->Port
			maskOr |= 0x1f140;
			break;
		default:
		...
		*(u32*)((usize)reg - (usize)Host + (usize)HostPhysical) = *((u32*)reg) & maskOr;
	} else if ((usize)reg == (usize)Power) {
		maskOr |= 0xffffffff;
...
void ReadBackReg(volatile const void* reg) {
	if ((usize)reg - (usize)Core < sizeof(struct CoreGlobalRegs)) {
		switch ((usize)reg - (usize)Core) {
		case 0x44: // Core->Hardware
			*((u32*)reg + 0) = *((u32*)((usize)reg - (usize)Core + (usize)CorePhysical) + 0);
			*((u32*)reg + 1) = *((u32*)((usize)reg - (usize)Core + (usize)CorePhysical) + 1);
			*((u32*)reg + 2) = *((u32*)((usize)reg - (usize)Core + (usize)CorePhysical) + 2);
			*((u32*)reg + 3) = *((u32*)((usize)reg - (usize)Core + (usize)CorePhysical) + 3);
			break;
		default:
			*(u32*)reg = *(u32*)((usize)reg - (usize)Core + (usize)CorePhysical);
		}
	} else if ((usize)reg - (usize)Host < sizeof(struct HostGlobalRegs)) {
		*(u32*)reg = *(u32*)((usize)reg - (usize)Host + (usize)HostPhysical);
	} else if ((usize)reg == (usize)Power) {
...
void SetReg(volatile const void* reg) {
	...
	if ((usize)reg - (usize)Core < sizeof(struct CoreGlobalRegs)) {
		...
		if ((usize)reg - (usize)Host > 0x100 && (usize)reg - (usize)Host < 0x300) {
			switch (((usize)reg - (usize)Host) & 0x1f) {
			case 0x8:
				value = 0x3fff;
				break;
			default:
...
Result HcdInitialise(void* hcd_designware_base) {	
	...
	if (Core->Hardware.Architecture != InternalDma) {
		LOG("HCD: (IGNORING!) Host architecture is not Internal DMA. Driver incompatible.\n");
//		result = ErrorIncompatible;
//		goto deallocate;
	}
	LOG_DEBUG("HCD: Internal DMA mode.\n");
	if (Core->Hardware.HighSpeedPhysical == NotSupported) {
		LOG("HCD: (IGNORING!) High speed physical unsupported. Driver incompatible.\n");
		// result = ErrorIncompatible;
		// goto deallocate;
	}
