...
Result UsbInitialise(void* hcd_designware_base) {
	Result result;
	...
	if ((result = HcdInitialise(hcd_designware_base)) != OK) {
...
Result UsbConfigure(struct UsbDevice *device, u8 configuration) {
	...
	for (header = (struct UsbDescriptorHeader*)((u8*)header + header->DescriptorLength);
		(usize)header - (usize)fullDescriptor < device->Configuration.TotalLength;
